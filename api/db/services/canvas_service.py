...
class UserCanvasService(CommonService):
    ...
    def get_all_agents_by_tenant_ids(cls, tenant_ids, user_id):
        ...
        fields = [cls.model.id, cls.model.title, cls.model.permission, cls.model.canvas_type, cls.model.canvas_category]
...
def completionOpenAI(tenant_id, agent_id, question, session_id=None, stream=True, **kwargs):
    ...
    if stream:
        completion_tokens = 0
        try:
            for ans in completion(tenant_id=tenant_id, agent_id=agent_id, session_id=session_id, query=question, user_id=user_id, **kwargs):
